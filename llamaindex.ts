 ...
 * ─── Storage ───────────────────────────────────────────────────────
 *   Persisted at:
 *     ~/.pi/Llamaindex/  (pi's config directory)
 *
 * Override via PI_LLAMAINDEX_DIR env var.
 */
...
interface QueryResult {
  ...
  file: string;
  fileName: string;
  /** Human-friendly display fields populated from node metadata */
  title?: string;
  category?: string;
  tags?: string;
  description?: string;
...
async function rerank(
  ...
  for (let i = 0; i < batchSize; i++) {
    ...
    const probs = softmax(row); // convert logits to probabilities
  ...
  try {
    if (key) {
      ...
      li.Settings.embedModel = new _hfEmbedding({
        ...
        // modelOptions is passed straight to Transformers.js pipeline()
        modelOptions: {
          quantized: true,
...
async function buildIndex(
  ...
  // Phase 1: Read files and parse into documents (fast — no real progress needed)
...
async function queryIndex(
  ...
  // With ~2000 indexed snippets, 60 is ~3% of the corpus — enough that
  // the cross-encoder in stage 2 rarely misses a relevant document.
  ...
  // Build candidate results for the reranker. Each node is a chunk
  // (~1024 chars from SentenceSplitter) with full metadata from the
  // source YAML frontmatter preserved on every chunk.
  const candidates: QueryResult[] = nodes.map((source: any) => {
    ...
    return {
      ...
      // Populate human-friendly fields from node metadata
  ...
  try {
    ...
    // A chunked file (SentenceSplitter) can contribute multiple nodes;
    // we show the reranker's top pick per file to avoid redundancy.
    ...
    // Fallback also deduplicates by file
...
export default async function (pi: ExtensionAPI) {
  ...
  async function cmdTags(_ctx: ExtensionCommandContext) {
    ...
    // Quick count: how many indexed files have each tag
    // We approximate by scanning state.indexedPaths (re-reading could be slow)
